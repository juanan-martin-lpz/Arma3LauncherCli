...
namespace ServidoresData
{
    public class Data
    {
        ...
        private void hacer_datatable()
        {
            ...
            // 
            c.CommandText = "create table if not exists " + tbname + " (mod text, ruta text, nombre text, firma text, tamano unsigned bigint)";
            c.ExecuteNonQuery();
        ...
        public int existFilename(string fname, string path)
        {

            string sql = "SELECT * FROM " +  tbname  + " WHERE RUTA = " + "'" + path + "'" + " AND NOMBRE = " + "'" + fname + "'";

            SQLiteDataAdapter a = new SQLiteDataAdapter(sql, con);

            DataTable t = new DataTable("conteo");
